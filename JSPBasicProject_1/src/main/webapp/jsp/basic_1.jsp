...
<%--
	...
	------
		Servlet => Server + let
					서버에서 실행하는 가벼운 프로그램
					=> 수정시마다 컴파일을 해야된다
					=> 순수하게 자바로만 코딩한다 => 순수 HTML 구현이 어렵다
	...
	=> JSP
		...
		=> 1. 웹 통작
			...
			3. 서버에서 해당 파일을 찾는다
				---------------------
				 html / xml / json => 웹 서버 자체 처리
				 jsp / servlet요청
				 ----------------웹서버는 처리를 못한다
				 | tomcat에 전송 => java 를 번역
				 | 메모리에 저장 (출력 버퍼)
	...
	5. JSTL
	6. MVC
	=> Spring : JSP / Spring-Boot : JSP ,TymeLeaf
